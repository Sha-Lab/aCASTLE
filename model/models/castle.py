...
class Castle(GeneralizedFewShotModel):
    ...
    def _forward(self, support, query, index_label):
        ...
        support = support[support_idx.flatten()].view(*(support_idx.shape + (-1,)))
        proto = support.mean(dim=1) # Ntask x NK x d
        num_proto = proto.shape[1]            
    
        # training with generalized classification (multiple tasks)       
        cls, _, _ = self.slf_attn(proto, self.shared_key.repeat(num_task, 1, 1), self.shared_key.repeat(num_task, 1, 1)) # num_task * num_way * dim
    
        # construct a task-specific global classifier for all tasks
        global_mask = torch.eye(self.args.num_class).repeat(1, num_task)
        global_mask_basis = torch.arange(num_task).view(-1,1).repeat(1, num_proto) * self.args.num_class
        ...
        current_classifier = torch.mm(self.cls.weight.t(), global_mask) + torch.mm(cls.t(), local_mask)
        current_classifier = F.normalize(current_classifier, dim=0)
        logit = torch.mm(query, current_classifier)
        logit = logit.view(-1, self.args.num_class)  / self.args.temperature   
        return logit   
    ...
    def _forward_fsl(self, support, query, aux=None):
        ...
        # use dict to obtain classifier
        cls, _, _ = self.slf_attn(proto.unsqueeze(0), self.shared_key, self.shared_key)
        cls = cls.squeeze(0)
        cls = F.normalize(cls, dim=1)            
        logit = torch.mm(query, cls.t())   / self.args.temperature  
        return logit   
    ...
    def _forward_gfsl(self, support_embs, query_embs, aux=None):
        ...
        cls_seen = F.normalize(self.cls.weight, dim=1)   
    ...
    def forward_many(self, data):
        cls_seen = F.normalize(self.cls.weight, dim=1)   
        return torch.mm(self.encoder(data), cls_seen.t())